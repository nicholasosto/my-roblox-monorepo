...
import { AssetType, CreateAssetRequest, UpdateAssetRequest, AssetOperationResponse } from '../types/assets';
import { OperationStatus } from '../types/common';

export class AssetsAPI {
  constructor(private client: OpenCloudClient) {}

  /**
   * Upload a new asset to Roblox
   */
  async createAsset(request: CreateAssetRequest): Promise<AssetOperationResponse> {
   ...
   */
  async updateAsset(assetId: string, request: UpdateAssetRequest): Promise<AssetOperationResponse> {
    const formData = new FormData();
   ...
   */
  async waitForOperation(operationId: string, pollInterval = 2000, maxAttempts = 30): Promise<OperationStatus> {
   ...
   */
  async uploadAssetAndWait(request: CreateAssetRequest): Promise<OperationStatus> {
